...
class TradingBot:
    ...
    def dry_run(self):
        ...
        if mt5_config.get("login"):
            print(f"    [OK] MT5 Login: {mt5_config['login']}")
            print(f"    [OK] MT5 Server: {mt5_config.get('server', 'Not set')}")
        else:
            print("    [WARN] MT5 credentials not configured")
        ...
        if self.mt5.connect(
            ...
            if account:
                ...
                print(f"    [OK] Balance: {account.balance} {account.currency}")
            self.mt5.disconnect()
        else:
        ...
        for name, strategy in strategies.items():
            for symbol in strategy.symbols:
                ...
                if data is not None:
                    print(f"    [OK] {symbol} {strategy.timeframe}: {len(data)} bars")
                else:
                    print(f"    [FAIL] {symbol}: Failed to get data")
        self.mt5.disconnect()
        ...
        if telegram.get("enabled"):
            print(f"    [OK] Telegram: Enabled (Chat ID: {telegram.get('chat_id', 'Not set')})")
        else:
