...
try:
    ...
    from indicators.smc_library_bridge import get_smc_analyzer
...
def render_smc_panel():
    """Render the Smart Money Concepts (SMC) live analysis."""
    ...
    if not connector or not connector.is_connected():
        st.info("Connect to MT5 to view live SMC analysis")
        return
        
    try:
        data_h1 = connector.get_ohlcv(symbol, "H1", 200)
        if data_h1 is None or data_h1.empty:
            st.warning(f"No H1 data available for {symbol}")
            return
            
        smc = get_smc_analyzer(point=0.1 if "XAU" in symbol else 0.0001)
        ...
        with col1:
            st.metric("Market Structure (H1)", struct.name, 
                     delta="↑" if struct.name == "BULLISH" else "↓" if struct.name == "BEARISH" else None,
                     delta_color="normal" if struct.name == "BULLISH" else "inverse" if struct.name == "BEARISH" else "off")
                     
        with col2:
            st.metric("Bullish FVG", f"{fvg_b.lower_price:.2f} - {fvg_b.upper_price:.2f}" if fvg_b else "None")
            
        with col3:
            st.metric("Bearish FVG", f"{fvg_s.lower_price:.2f} - {fvg_s.upper_price:.2f}" if fvg_s else "None")
            
        with col4:
            st.metric("Bullish OB", f"{ob_b.lower_price:.2f} - {ob_b.upper_price:.2f}" if ob_b else "None")
            
        with col5:
            st.metric("Bearish OB", f"{ob_s.lower_price:.2f} - {ob_s.upper_price:.2f}" if ob_s else "None")
            
    except Exception as e:
        st.error(f"Error analyzing SMC: {e}")
